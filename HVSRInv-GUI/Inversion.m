function [parameters] = Inversion( layerNum,invNum,iterNum,popNum,freqs,HVSR,freqMin,freqMax,sampleNum,initModData,handle)
...
for i=1:layerNum
    S=initModData(i,:);    
    param(k,1)=S(4);param(k,2)=S(5);param(k,3)=S(6);
    k=k+1;
    param(k,1)=S(7);param(k,2)=S(8);param(k,3)=S(9);
    k=k+1;
    param(k,1)=S(10);param(k,2)=S(11);param(k,3)=S(12);
    k=k+1;
    if i<layerNum
        param(k,1)=S(1);param(k,2)=S(2);param(k,3)=S(3);
        k=k+1;
    end;
end;
